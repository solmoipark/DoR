...
| G3-3 실측 66배합 | **최종: 전체 격하 유지, 슬래그는 MAE 기준 충족(조건부)** | 최종 실행 v3 = 머지 커널(P-IG-6 + CEM I) + 최종 단위 규칙 + 우도 정책(BW 비례 0.60, CH 가중 0.25/−3 g) + α 격자 11: 사용 가능 13배합(66후보 중 53배합은 등급 A/B 관측 없음·재구성 불가), 82 DoR 점. **전체 MAE 13.4 %p, 90 % 포함률 0.62, 편향 −13.0** → 스펙 기준(MAE < 10.3 또는 포함률 ≥ 0.8) 미달 → 격하 유지. **슬래그**(1편, 9배합, 54점): MAE **8.7 %p**(< 10.3 충족), 포함률 0.70, 편향 −8.7 — 오차의 대부분은 1–2 d 조기 재령(예: 측정 20 % vs 사후 10 %)이고 28 d는 ±5 %p 안. **메타카올린**(1편, 4배합, 28점): MAE 22.3, ESS 2000 = 관측이 우도에 정보를 주지 못함(사전분포 그대로). 이력: 옛 커널 16.2/0.24 → 머지 커널·엄격 등급 22.7/0 → v2(가중) 16.1/0.55 → v3(비례 보정) 13.4/0.62. 결론: (i) 정책은 옳은 방향(BW 비례 보정이 슬래그를 12.9→8.7로), (ii) 남은 편향은 조기 재령 — 고정 β = 0.5의 stretched-exp 사전과 클링커 α 미고정(P-IG-3)이 후보, (iii) 문헌 관측 부족이 근본 한계. 상태: `inverse.status = consistency_check`, 슬래그는 `provisional` | `docs/real/merged/g3_3_v3_final.json`, `g3_3_v3_points.csv` |
...
| ID | 브랜치 | 상태 |
|---|---|---|
| P-IG-1 | `InverseGems: dorgems/p-ig-1-materials-config` (`82df677`) → **로컬 master 머지(`f8ce7be`)** | 완료. DoRGems `run_forward`가 `materials_injection: native`로 전환됨(monkeypatch 폴백은 유지). `materials_config`를 forward/task/design 경로에 스레딩, `load_materials()` 6곳 교체, `recipe.json` metadata에 경로 기록, 테스트 1개 추가. 업스트림 테스트 222 passed / 1 failed(기존 `test_feature_table` 실패, 패치 무관). **DoRGems 확인:** 이 브랜치를 체크아웃하면 `run_forward`가 `materials_injection: native`로 전환되고 자기검증(alpha_ok·materials_ok) 통과 |
| P-GP-1 | `GemsPilot: dorgems/p-gp-1-reaction-model-kwargs` (`f0da5e8`) → **로컬 main 머지(`9fc98ce`)** | 완료. `run_forward/run_task/run_confirmed_query/run_design_with_recovery`와 MCP 래퍼에 `reaction_model_config`·`reaction_model_id`·`materials_config` 추가(커널 시그니처를 검사해 조건부 전달; 커널이 못 받는 non-None 옵션은 명시적 오류). `design_recovery`의 `None` 하드코딩 인자화. 52 passed / 1 failed(기존 하드코딩 경로 실패) |
| P-GP-2 | `GemsPilot: dorgems/p-gp-2-toolset-entrypoints` (`ee3afbb`) → **로컬 main 머지(`87594e8`)** | 완료. GemsPilot `default_toolset()`이 DoRGems 엔트리포인트로 `dor_*` 13개를 자동 발견(총 28개), MCP 서버도 자동 등록. `default_toolset(extra, discover=True)` + `gemspilot.toolsets` 엔트리포인트 탐색(고장난 엔트리포인트는 경고로 수집), `mcp_server.register_extra_toolsets`. 이 환경에서는 DoRGems 엔트리포인트가 실제로 잡혀 도구 15→28개(MCP 31개). 53 passed / 1 failed(기존) |
| P-GP-3 | `GemsPilot: dorgems/p-gp-3-write-policy` (`9f54ad1`) → **로컬 main 머지(`57cac51`)** | 완료. DoRGems `dor_stage_inferred`는 정책 `write`·인자 `dry_run`으로 개명(`use_mock` 별칭 유지). `write` 정책: `dry_run` falsy + `allow_real` 없음 → 거부. 49 passed / 1 failed(기존). 개명 완료 |
