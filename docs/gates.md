...
| G1-5 MCP 한 에피소드 | **부분(LLM 키 대기)** | `scripts/run_llm_episode.py`가 GemsPilot 러너 + DoR 툴셋으로 하위 모델(기본 `openrouter/anthropic/claude-haiku-4.5`) 에피소드를 실행·채점하도록 준비됨; `OPENROUTER_API_KEY`가 없어 미실행(`needs_api_key` 기록). 이전: | `dorgems-mcp` 서버 구현 + `dor_run_envelope`가 predict→export→override→forward×3를 한 호출로 완료(mock, CLI로 실증). LLM 호스트가 실제로 연결된 에피소드는 미실행(API 키 없음) | `tests/test_forward_mock.py`, CLI `dorgems envelope` |
