...
| G2-5 twin ≥ 20 배합 | **실행 완료(63배합, 실기) — 판정 대부분 데이터 부족** | 실제 xGEMS, 측정 DoR pin: 74후보 중 63배합 실행(11배합은 w/b·T 결측). 최종 단위 규칙으로 등급 A 관측은 **106건/17배합/3편**뿐(D 111, C 90, X 37). 판정: insufficient_data 46 / tension 13 / consistent 4. **bound_water**(1편, 4배합, n=28): median r +2.4 g, IQR 3.8 → 4배합 consistent. **CH_TGA**(2편, 13배합, n=78): median r −5.0 g, 관측 중앙 6.1 g vs 모델 중앙 **0.0 g**(51 %의 관측점에서 모델 CH = 0) → tension 13. 1차 집계(규칙 결함 전, 10편)에서도 같은 그림(BW median r −1.9, CH 모델 0). 해석: 측정 DoR(선택용해·SEM 등)을 그대로 고정하면 열역학 모델은 포틀란다이트를 전부 소비하는데 논문은 CH를 계속 검출 — (i) 측정 DoR의 열역학적 과대, (ii) CH 가용성 보정 비활성(§6.1 설계), (iii) C-S-H Ca/Si 흡수량 중 무엇이 원인인지는 커널 측 과제. 등급 A 관측 수가 적은 것은 §14-1 basis 백로그 그대로 | `docs/real/g2_5_twin_batch.*` |
...
| G3-3 실측 66배합 | **예비 결과: 격하 방향(정식 판정은 머지 커널 재실행 후)** | 옛 커널(`e84d7a9`)·수정 전 단위 규칙, 66후보 중 48배합 실행(38 성공/7편, 159 DoR 점; 10건은 재구성 불가·등급 A/B 관측 없음): 숨긴 DoR 대비 사후 중앙값 **MAE 16.2 %p**, 90 % 포함률 **0.24**, 편향 −7.3 %p. 슬롯별: fly_ash MAE 8.8(GBM 10.3보다 낮음, 포함률 0.42, 편향 +6.8), slag MAE 23.4(편향 −23.4, 포함률 0), metakaolin 36.8(편향 −36.8). 해석: 슬래그·MK에서는 CH 관측을 맞추려면 측정 DoR보다 훨씬 낮은 α가 필요 — twin(G2-5)의 "측정 DoR 고정 시 모델 CH 0"과 같은 현상. 스펙 기준(MAE < 10.3 또는 포함률 ≥ 0.8)에 미달 → **시나리오 C는 '일관성 점검·불확실성 축소' 용도로 격하** 예정, 클링커 α 고정(P-IG-3)과 CH 우도 편향(커널 조사 참조)을 원인 후보로 기록. 머지 커널 + 최종 단위 규칙 재실행 진행 중(`work/real/g3_3_merged`) | `docs/real/g3_3_prelim_base_kernel.json`, `scripts/g3_3_real_hidden_dor.py` |
